...
grid_rows = 3
grid_columns = 4
fig, all_axes = plt.subplots(grid_rows, grid_columns, figsize=(9*grid_columns, 7*grid_rows), sharey=True, sharex=False, constrained_layout=True) #, dpi=600)
i = 0
for idx_i in range(grid_rows):
    for idx_j in range(grid_columns):
        ...
        for locus_count, p in zip(row_values_seaborn_i["Count"], axes.patches):
            if locus_count != 0 and locus_count not in largest_5_values:
                continue
            x_pos = p.get_x() + 0.5
            y_pos = p.get_height()
            axes.annotate(f"{int(locus_count):,d}", (x_pos, y_pos),
                             ha="center", va="bottom", fontsize=11, color="black", xytext=(0, 10),
