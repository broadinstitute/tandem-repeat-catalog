...
catalogs_in_order = [
	...
	("polymorphic_loci_in_HPRC_assemblies", "https://storage.googleapis.com/str-truth-set-v2/filter_vcf/all_repeats_including_homopolymers_keeping_loci_that_have_overlapping_variants/combined/combined.78_samples.positive_loci.json"),
	...
	("UCSC_simple_repeat_track", "https://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/simpleRepeat.txt.gz"),
]
...
if "UCSC_simple_repeat_track" in catalog_paths and (
	not args.keyword or args.keyword.lower() in "UCSC_simple_repeat_track".lower() or args.keyword.lower() in catalog_paths["UCSC_simple_repeat_track"].lower()):
	path_after_conversion = catalog_paths["UCSC_simple_repeat_track"].replace(".txt.gz", "") + "_track_from_UCSC.bed.gz"
	if not os.path.isfile(path_after_conversion):
		run(f"python3 -u {scripts_dir}/convert_ucsc_simple_repeat_track_to_bed.py {catalog_paths['UCSC_simple_repeat_track']} -o {path_after_conversion}")
	catalog_paths["UCSC_simple_repeat_track"] = path_after_conversion

# convert Chiu et al catalog to ExpansionHunter catalog format for comparison
if "comprehensive_catalog_from_Chiu_et_al" in catalog_paths and (
	not args.keyword or args.keyword.lower() in "comprehensive_catalog_from_Chiu_et_al".lower() or args.keyword.lower() in catalog_paths["comprehensive_catalog_from_Chiu_et_al"].lower()):
...
for catalog_name, path in catalog_paths.items():
	...
	if not os.path.isfile(annotated_catalog_path) or args.force:
		run(f"""python3 -m str_analysis.annotate_and_filter_str_catalog --verbose \
